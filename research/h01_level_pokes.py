...
def md(df):
    ...
    label = lambda x: " / ".join(map(str, x)) if isinstance(x, tuple) else str(x)  # noqa: E731
    cols = [label(df.index.names if df.index.nlevels > 1 else df.index.name or "")] + [label(c) for c in df.columns]
